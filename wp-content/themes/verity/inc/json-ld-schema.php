 ...
 * @since Verity
 */
if( !class_exists('Verity_Json_Ld_Schema')){
class Verity_Json_Ld_Schema {
	private $position        = 0;
	...
	function verity_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'verity' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'verity' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'verity' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'verity' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'verity' ); // text for an author page
		$text['404']      = __( 'Error 404', 'verity' ); // text for the 404 page
	...
	function json_ld_breadcrumbs() {
		...
		if ( get_theme_mod( 'verity_breadcrumb_option', 1 ) ) {
			$show_on_home = true;

			$breadcrumb = $this->verity_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
